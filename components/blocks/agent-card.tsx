...
export function AgentCard({ agent, featured = false, className }: AgentCardProps) {
  return (
    <>
      ...
      {/* Full card — unchanged design, from sm upward */}
      <Link
        href={`/agents/${agent.slug}`}
        className={cn(
          "hidden flex-col gap-4 rounded-xl border border-muted/40 bg-background-100 p-6 transition-colors hover:border-muted sm:flex",
          className
        )}
      >
